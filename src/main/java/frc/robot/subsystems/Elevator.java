    ...
    */
    ...
    public void setGoal() {
        ...
        switch (elevatorState) {
            case START_LEVEL:
                m_controller.setGoal(kToEndOfBelt); // does k.end mean ktoendofbelt?
                m_controller2.setGoal(kRest);

                elevatorState = Level.FIRST_LEVEL;
                break;

            case FIRST_LEVEL:
                m_controller.setGoal(kExitBelt);// controller for the entry motor will set just enough for the ball to
                                                // exit
                // the first belt onto the second
                m_controller2.setGoal(kToEndOfBelt);// controller for the exit motor will set just enough for the ball
                                                    // to
            ...
            case SECOND_LEVEL:
                m_controller2.setGoal(kExitBelt);
                m_controller.setGoal(kRest);
         ...
         * elevatorState = Level.FIRST_LEVEL;
         */
        // motorController.setGoal
        // } else if (elevatorState == Level.FIRST_LEVEL) {
        // this section is completed. use it as an example to finish the other two
        // conditions
        /*
         * m_controller.setGoal(kExitBelt);// controller for the entry motor will set
         * just enough for the ball to exit
         * // the first belt onto the second
         * m_controller2.setGoal(kToEndOfBelt);// controller for the exit motor will set
         * just enough for the ball to
         * // reach the end of the second belt
         * 
         * elevatorState = Level.SECOND_LEVEL;// updates the elevator state to be the
         * next level
         * 
